/**
 * A self-contained implementation of the classic Perlin Noise algorithm for 1D, 2D, and 3D.
 * Based on Ken Perlin's "Improved Noise" reference implementation.
 *
 * All noise functions return a value in the approximate range of [-1, 1].
 */
const PerlinNoise = (() => {

  // The permutation table, shuffled once and kept constant.
  ...
  // The permutation table is duplicated to avoid expensive modulo operations.
  const perm = new Uint8Array(512);
  for (let i = 0; i < 512; i++) {
    perm[i] = p[i & 255];
  }

  // A smooth interpolation curve (6t^5 - 15t^4 + 10t^3).
  ...
  return { noise1D, noise2D, noise3D };
})();

export const { noise1D, noise2D, noise3D } = PerlinNoise;
