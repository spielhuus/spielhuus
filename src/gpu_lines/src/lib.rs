...
impl State {
    ...
    fn render(&mut self) -> Result<(), wgpu::SurfaceError> {
...

        {
            ...
            if self.position[0] + self.accel < 0.0
                || self.position[0] + self.accel > self.window.inner_size().width as f32
                || self.position[1] + self.accel < 0.0
                || self.position[1] + self.accel > self.window.inner_size().height as f32
            {
                self.accel *= -1.0;
            }

            self.position = [self.position[0] + self.accel, self.position[1] + self.accel];
