...
fn get_transformation_matrix_idiomatic(projection_matrix: Mat4, elapsed: &f32) -> Mat4 {
    let camera_position = Vec3::new(3.0, 4.0, -5.0);
    let camera_target = Vec3::ZERO;
    let up_direction = Vec3::Y;
    let view_matrix = Mat4::look_at_rh(camera_position, camera_target, up_direction);

    let rotation_axis = Vec3::new(elapsed.sin(), elapsed.cos(), 0.0).normalize_or_zero();
    let model_matrix = Mat4::from_axis_angle(rotation_axis, 1.0);
...
impl State {
    async fn new(
        ...
        let shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("Shader"),
            source: wgpu::ShaderSource::Wgsl(include_str!("vertex_buf_shader.wgsl").into()),
        });

        let aspect_ratio = config.width as f32 / config.height as f32;
        let projection_matrix = Mat4::perspective_rh_gl(
            45.0f32.to_radians(), // fovy
            aspect_ratio,
            0.1,   // near plane
            100.0, // far plane
        );
    ...
    fn render(&mut self) -> Result<(), wgpu::SurfaceError> {
...

        {
            ...
            let view = glam::Mat4::look_at_rh(
                // glam::Vec3::new(1.5, -2.5, 3.0),
                glam::Vec3::new(1.0, 1.0, 1.0),
                glam::Vec3::ZERO,
                glam::Vec3::Y,
            );

            let scale_factor = 0.2; // Adjust this value to change the size of the cube
            let scale_matrix =
                glam::Mat4::from_scale(glam::Vec3::new(scale_factor, scale_factor, scale_factor));
            let model = glam::Mat4::from_rotation_y(elapsed) * scale_matrix;

            let transform_matrix = projection * view * scale_matrix; // * model;
